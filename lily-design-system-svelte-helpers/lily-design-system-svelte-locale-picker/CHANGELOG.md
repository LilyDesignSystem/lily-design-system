...
`class` (e.g. `locale-picker-list`) was never actually set, so a
...
**Package renamed: `lily-design-system-svelte-locale-picker` → `@lilydesignsystem/svelte-locale-picker`.** npm scoped packages
...
relative to `lily-design-system-svelte-locale-picker`'s last published version (`0.2.0`);
...
- **Applying the same locale twice is now a no-op**, so `onChange` fires
  ...
  can run for reasons other than a locale change; when it did, it
...
- Version reset to `0.1.0`. Nothing has been published under the name
  `@lilydesignsystem/svelte-locale-picker`, so continuing the old
...
- Three new tradeoffs are documented honestly in
  ...
  rests entirely on `aria-label` — which for a _locale_ select is
