...
```svelte
<script lang="ts">
  import LocalePicker, { bcp47LocaleTag, localeName } from "../LocalePicker.svelte";
  let locale = $state("");
</script>
...
```ts
import { isRtlLocale } from "../LocalePicker.svelte";
...
```svelte
<script lang="ts">
  import LocalePicker, { defaultLocaleLabels } from "../LocalePicker.svelte";
  const ALL = Object.keys(defaultLocaleLabels);
</script>

<LocalePicker label="Language" locales={ALL} bind:value={locale} />
```
...
## Pair with theme-picker in a header
...
text-presentation font on `.locale-picker-icon`, or swap in an SVG.
