...
async function openWith(key: string) {
    render(ThemeSelect, { props: { label: "Theme", themesUrl: "/t/", themes: THEMES } });
    await flush();
    const button = screen.getByRole("button");
    await fireEvent.keyDown(button, { key });
    await flush();
    return { button, list: document.querySelector(".theme-select-list") as HTMLElement };
}
```
