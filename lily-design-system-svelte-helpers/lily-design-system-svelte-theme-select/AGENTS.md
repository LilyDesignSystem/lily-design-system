...
consumer styles the `theme-select` class hooks (and must supply the
...
- Module-script exports: `normaliseThemesUrl`, `themeHref`,
  `themeName`, `matchSystemTheme`, `nextThemeSelectId`,
  `CIRCLE_WITH_RIGHT_HALF_BLACK`.
- Barrel (`index.ts`) currently re-exports only `default`,
  `ThemeSelect`, `normaliseThemesUrl`, `themeHref`, and the types —
