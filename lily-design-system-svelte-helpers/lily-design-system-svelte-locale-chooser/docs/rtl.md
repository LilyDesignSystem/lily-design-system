...
detected, what doesn't, and what `dir="rtl"` actually changes in the
browser.
...
- **Kurdish (Latin)** — `ku-Latn` (Kurmanji, written in Latin) is
  ...
  yourself, or rename the locale to a script-explicit form like
  `kmr` (Northern Kurdish, ISO 639-3) which is not in the RTL set.
