...
```ts
import { defaultLocaleLabels } from "../LocaleSelect.svelte";
...
This mirrors `detectFromSystem` in `theme-select`. The underlying
...
Unlike `theme-select`'s `name`, it has no second job — there is no
...
When supplied, no `.locale-select-icon` span is emitted. The
