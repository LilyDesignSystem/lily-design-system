...
```css
.locale-select-option:has(input:checked) {
...
LTR. So Tab still moves through the radios in the order they appear in
your `locales` array, and Arrow Right (in RTL) moves to the previous
option, not the next. This is the browser's job, not yours.

## References

- WAI-ARIA APG — Radio Group pattern:
  <https://www.w3.org/WAI/ARIA/apg/patterns/radio/>
- WAI-ARIA APG — Combobox pattern (for the children snippet case):
