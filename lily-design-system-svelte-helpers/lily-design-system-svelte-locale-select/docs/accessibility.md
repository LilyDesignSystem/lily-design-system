...
- The name is invisible to sighted users. The globe glyph is a
  ...
  this compared to `theme-select`'s `◑` — but a convention is not a
  label.
...
- **The variation selector may be ignored.** VS15 *requests* text
  ...
  regardless, which is precisely the mismatch with `theme-select`'s
...
- **Set an explicit font stack** on `.locale-select-icon` that you have
...
- **`.locale-select-status` is the class hook**, in the same kebab-case
