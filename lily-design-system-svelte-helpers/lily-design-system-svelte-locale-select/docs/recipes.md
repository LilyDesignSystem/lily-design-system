...
```svelte
<script lang="ts">
  import LocaleSelect, { bcp47LocaleTag, localeName } from "../LocaleSelect.svelte";
  let locale = $state("");
</script>
...
```svelte
<LocaleSelect label="Language" locales={["en", "ar"]} applyDir={false} />
```
...
```ts
import { isRtlLocale } from "../LocaleSelect.svelte";
...
```svelte
<script lang="ts">
  import LocaleSelect, { defaultLocaleLabels } from "../LocaleSelect.svelte";
  const ALL = Object.keys(defaultLocaleLabels);
</script>

<LocaleSelect label="Language" locales={ALL} bind:value={locale} />
```
...
## Pair with theme-select in a header
...
<LocaleSelect label="Language" {locales} bind:value={locale} />
```
...
text-presentation font on `.locale-select-icon`, or swap in an SVG.
