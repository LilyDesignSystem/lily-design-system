...
**Package renamed: `lily-design-system-svelte-picker-bar` → `@lilydesignsystem/svelte-picker-bar`.** npm scoped packages
...
relative to `lily-design-system-svelte-picker-bar`'s last published version (`0.1.1`).
This is this package's first dedicated `CHANGELOG.md`; its prior
history (as `lily-design-system-svelte-picker-bar`) is recorded in the root
