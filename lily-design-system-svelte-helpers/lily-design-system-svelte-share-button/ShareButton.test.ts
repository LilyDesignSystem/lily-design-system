...
describe("ShareButton — markup contract (§7.1–§7.6)", () => {
    test("§7.1 renders a disclosure button controlling a list", () => {
        render(ShareButton, { props: { label: "Share", targets: TARGETS, url: URL_UNDER_TEST } });
    ...
    test("§7.3 destinations are real links, not role=menuitem", async () => {
        render(ShareButton, { props: { label: "Share", targets: TARGETS, url: URL_UNDER_TEST } });
        await fireEvent.click(screen.getByRole("button", { name: "Share" }));
        const links = document.querySelectorAll<HTMLAnchorElement>(".share-button-target");
...
describe("ShareButton — copy to clipboard (§7.7–§7.10)", () => {
    test("§7.7 copying writes the URL and fires onCopy", async () => {
        ...
        await fireEvent.click(screen.getByRole("button", { name: "Share" }));
        await fireEvent.click(document.querySelector(".share-button-copy")!);
        await flush();
    ...
    test("§7.8 a successful copy announces copiedLabel and closes the list", async () => {
        ...
        expect(
            (document.querySelector(".share-button-list") as HTMLElement).hasAttribute("hidden"),
        ).toBe(true);
        clip.restore();
    });
...
describe("ShareButton — keyboard and dismissal (§7.15–§7.19)", () => {
    async function openList() {
        ...
        await flush();
        return { button, list: document.querySelector(".share-button-list") as HTMLElement };
    }

    test("§7.15 opening moves focus to the first item", async () => {
        const { list } = await openList();
        expect(document.activeElement).toBe(list.querySelector(".share-button-target"));
    });
...
describe("ShareButton — url resolution and custom glyph (§7.20–§7.22)", () => {
    test("§7.20 an explicit url prop wins", async () => {
        render(ShareButton, { props: { label: "Share", targets: TARGETS, url: URL_UNDER_TEST } });
        await fireEvent.click(screen.getByRole("button", { name: "Share" }));
        const href = document.querySelector(".share-button-target")!.getAttribute("href")!;
    ...
    test("§7.21 with no url prop it falls back to the current page URL", async () => {
        render(ShareButton, { props: { label: "Share", targets: TARGETS } });
        await fireEvent.click(screen.getByRole("button", { name: "Share" }));
        const href = document.querySelector(".share-button-target")!.getAttribute("href")!;
