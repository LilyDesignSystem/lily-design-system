...
the control's _entire_ accessible name — there is no visible text to
...
**Replaces the glyph inside the trigger button.** It does _not_ render
...
_button_ (such as `aria-labelledby`) land on the root, not the button
