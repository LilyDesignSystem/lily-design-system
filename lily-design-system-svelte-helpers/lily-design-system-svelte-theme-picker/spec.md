...
- Root element: `<fieldset class="theme-picker {class}" role="radiogroup"
  aria-label="{label}">`.
- Default children: one `<label class="theme-picker-option">` per theme
...
`ThemePicker.test.ts` must assert every numbered item below. Tests run
