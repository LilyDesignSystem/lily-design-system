...
describe("LocaleChooser — markup contract (§4.3, §7.1)", () => {
    ...
    test("§7.1 the button renders the globe glyph, hidden from assistive tech", () => {
        ...
        // and matches theme-chooser's ◑ rather than the blue emoji.
    ...
    test("§7.1 the root is a div carrying the class hook", () => {
        const { container } = render(
            <LocaleChooser label="Language" locales={LOCALES} className="mine" />,
        );
        const root = container.firstElementChild as HTMLElement;
        expect(root.tagName).toBe("DIV");
        expect(root.className).toBe("locale-chooser mine");
    });

    test("§7.2 aria-label names the button and the listbox", () => {
        render(<LocaleChooser label="Choose language" locales={LOCALES} />);
        expect(
    ...
    test("§7.3 one option per locale; the hidden input carries the supplied name", async () => {
        render(<LocaleChooser label="Language" locales={LOCALES} name="lang" />);
        await flush();
    ...
    test("§7.6 falls back to defaultLocaleLabels when localeLabels missing", () => {
        render(<LocaleChooser label="Language" locales={["en_US"]} />);
...
describe("LocaleChooser — spread + custom children (§4.1, §7.5)", () => {
    test("§7.22 extra attributes spread onto the root div", () => {
        render(
            <LocaleChooser label="Language" locales={LOCALES} data-testid="lp" />,
        );
        const root = screen.getByTestId("lp");
        expect(root.tagName).toBe("DIV");
        expect(root.className).toContain("locale-chooser");
    });
