...
contract — the full existing test suite (60 tests) passes unchanged, run against
...
**Package renamed: `lily-design-system-react-locale-picker` → `@lilydesignsystem/react-locale-picker`.** npm scoped packages
...
relative to `lily-design-system-react-locale-picker`'s last published version (`0.2.0`);
...
`LocalePicker` is a headless React 19 locale control: an icon button that
...
consumer styles the `locale-picker`, `locale-picker-button`,
...
Public surface: `LocalePicker` (default and named), `localeName`,
...
- Previously released in-tree as `lily-design-system-react-locale-select`.
  ...
  to `LocalePicker`, the `locale-picker*` class hooks to
  `locale-picker*`, and the `data-lily-locale-select*` attributes to
  `data-lily-locale-picker*`. It brings the helper into line with the
...
- **The default globe glyph gains U+FE0E VARIATION SELECTOR-15.**
  ...
  match theme-picker's monochrome `\u25D1` when the two controls sit
...
- **`docs/props-reference.md`, `docs/styling.md`,
  ...
  `docs/troubleshooting.md`** — the five topic guides theme-picker
  already shipped, written for locale-picker. The package's
...
- **The control is no longer a native `<select>`.** It is now an icon
  ...
  `<div class="locale-picker {className}">` — rest props spread onto
  that div — containing a hidden input, a
  `<button class="locale-picker-button">` with `aria-haspopup="listbox"`,
  `aria-expanded`, and `aria-controls`, and a
  `<ul class="locale-picker-list" role="listbox">` of
  `<li class="locale-picker-option" role="option">` children. The
  button holds `<span class="locale-picker-icon" aria-hidden="true">`
...
- **The `placeholder` prop is REMOVED**, along with the whole 0.3.0
  placeholder-pinning design and the `locale-picker-placeholder` class
...
- `name` is now the `name` of the hidden input rather than of the
  `<select>`.
- Class-hook contract: `locale-picker` names the root `<div>`; the new
  hooks are `locale-picker-button`, `locale-picker-icon`,
...
- `locale-picker-placeholder` class hook on the placeholder `<option>`
  (alongside `locale-picker-option`).
...
This helper mirrors
`lily-design-system-svelte-helpers/@lilydesignsystem/svelte-locale-picker`:
