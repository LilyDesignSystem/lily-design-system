...
| `children`            | `(args: ChildArgs) => React.ReactNode`  | no       | `<span class="locale-select-icon">🌐</span>` | Render prop that **replaces the glyph inside the button**. It does not render options — the component owns those. |
...
The returned nodes are rendered inside `<button class="locale-select-button">`
in place of the default `<span class="locale-select-icon">`. Because the
...
- **Root.** `<div className="locale-select {className}">`. All rest
...
- **Glyph.** By default `<span class="locale-select-icon"
  ...
  theme-select's `\u25D1`. It is hidden from
...
- **Listbox.** `<ul class="locale-select-list" role="listbox"
...
1. Renders a `<button type="button">` with `aria-haspopup="listbox"`,
   ...
   `.locale-select-icon`, marked `aria-hidden="true"`.
   The root is a `<div>` whose class is `locale-select {className}`.
...
23. A custom `children` render prop replaces the default glyph inside
    `.locale-select-button` (no `.locale-select-icon` is rendered) and
