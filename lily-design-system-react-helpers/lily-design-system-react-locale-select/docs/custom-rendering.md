...
`<button class="locale-select-button">` in place of the default
`<span class="locale-select-icon">`. Note the consequence: when you pass
`children`, the `.locale-select-icon` hook is **not** in the DOM. Every
...
```tsx
<LocaleSelect label="Language" locales={locales} localeLabels={endonyms}>
    {({ value, labelFor }) => (
        <>
...
```tsx
<LocaleSelect label="Language" locales={["en", "fr", "de", "ja"]}>
    {({ value }) => (
...
`.locale-select-button[aria-expanded="true"]` and skip the render prop
...
<>
    ...
    <p className="locale-select-status">
