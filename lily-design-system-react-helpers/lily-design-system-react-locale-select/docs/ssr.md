...
3. JS hydrates, `LocaleSelect` runs its `useEffect`, reads
...
a client component (the select wrapper).
...
### Client component (select wrapper)
...
The select is unchanged. If you need RFC 4647-quality negotiation,
...
is to mount the select with `value=` matching what the server sent.
...
select. Server and client agree.
...
Don't import `LocaleSelect` (the React component) from a server
