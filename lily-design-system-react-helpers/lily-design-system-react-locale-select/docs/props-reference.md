...
<LocaleSelect label="Langue" locales={locales} />
<LocaleSelect label="اللغة" locales={locales} />
```
...
<LocaleSelect value={locale} onChange={setLocale} label="Language" locales={locales} />

// Uncontrolled — fine when the DOM `lang`/`dir` is the only consumer.
<LocaleSelect defaultValue="fr" label="Language" locales={locales} />
```
...
```tsx
<LocaleSelect storageKey="my-app:locale" label="Language" locales={locales} />
```
...
<div ref={panelRef}>
    <LocaleSelect target={panelRef.current} label="Language" locales={locales} />
</div>
```
...
```tsx
<LocaleSelect applyDir={false} label="Language" locales={locales} />
```
...
Appended to the root's `locale-select` class hook, giving
`class="locale-select your-class"`. The base hook is never replaced.
