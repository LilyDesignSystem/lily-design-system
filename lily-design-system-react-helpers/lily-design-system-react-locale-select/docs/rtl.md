...
The select auto-detects right-to-left locales and writes `dir="rtl"` to
...
- You manage `dir` server-side and don't want the select to clobber it
  on hydration.
...
The same predicate the select uses internally is exported for consumer
...
1. **Manual** — pick an RTL locale in the select and visually verify
...
- The select writes `dir` inside `useEffect`. SSR output has whatever
