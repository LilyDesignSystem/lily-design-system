...
This page shows how to wire the select to the four most common React
...
through context. Drive the provider's `locale` prop from the select.
...
loaded; the select calls it from `onChange` and `<Trans>` re-renders
...
the select from `useParams()` and `onChange` calls `router.push()`:
...
A page can host several `LocaleSelect` instances (e.g. one global, one
scoped to a panel). Give each a different `name` prop so the
form-control identities don't collide:

```tsx
<LocaleSelect label="Page" name="page-locale" {...} />
