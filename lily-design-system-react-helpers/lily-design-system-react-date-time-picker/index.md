# Lily Design System™ — React DateTimePicker

A headless React 19 control for collecting a **date**, a **time**, or
...
Pass `value` + `onChange` (as above) to control it from your own state, or
omit `value` entirely and the component manages its own copy internally,
still reporting every change through `onChange`:

```tsx
<DateTimePicker
  label="Choose an appointment date"
  labels={labels}
  onChange={(iso) => console.log("committed:", iso)}
/>
```

This is the same `isControlled` convention `ThemePicker` and `LocalePicker`
...
```tsx
<DateTimePicker label={label} labels={{ ...labels, week: "Wk" }} showWeekNumbers />
```
...
`children` is a render prop — it replaces the glyph inside the button, not
the dialog.
...
and every DOM read/write happens inside `useEffect`. Under the Next.js App
Router, mark the importing module `"use client"` — the control is
interactive.
...
catalog root — 58 cases, one or more per §7 clause.
