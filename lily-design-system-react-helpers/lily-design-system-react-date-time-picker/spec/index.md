...
`aria-invalid="true"`, and fires `onInvalidInput`.
...
on a vetoed day inside the range, but may not leave the `min`/`max` window
at all.
...
leaving focus on an unrendered cell. The cells are real focusable
`<button>` elements — no `aria-activedescendant` — matching `share-picker`
rather than the three listbox helpers.
...
title carries its clause number. 58 tests total, using vitest + jsdom +
