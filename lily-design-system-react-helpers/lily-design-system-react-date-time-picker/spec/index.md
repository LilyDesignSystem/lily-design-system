...
| `timeZone` | `string` | no | `""` | Selected IANA zone, or `""` for none. Supply with `onTimeZoneChange` to control it. Rides `{name}-time-zone` and `data-time-zone`. See §5.9. |
...
- **Week and day move the pending day.** The cursor steps ±7 / ±1 civil
  ...
  onto a vetoed day moves the cursor (vetoed days are reachable, as with
  the arrow keys) but leaves the pending selection where it was. A step
  never commits, even under `confirmOnSelect`.
...
- The selected zone follows the same controlled/uncontrolled split as
  `value`: supplying `timeZone` makes it controlled and requires
  `onTimeZoneChange` to see it change. It rides its own hidden input,
  `{name}-time-zone`, and is reflected as `data-time-zone` on the root
  (absent while empty).
...
| time-zone `<label>` / `<select>` | `htmlFor` → the select's id; the label's text is `labels.timeZone` | Component + consumer |
