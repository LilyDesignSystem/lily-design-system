# AGENTS — SharePicker (React helper)
...
| `SharePicker.tsx`      | Implementation. React 19 hooks + TypeScript. |
| `SharePicker.test.tsx` | Vitest spec, mapped to the §7 clauses.       |
...
- The list id comes from `React.useId()`, as in the sibling helpers, so
  it survives hydration. `nextSharePickerId()` is exported for parity
...
- Unlike the `*-picker` siblings there is no `aria-activedescendant`:
