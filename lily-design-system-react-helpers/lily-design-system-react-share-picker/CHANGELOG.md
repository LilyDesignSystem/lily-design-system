...
keyboard contract — the full existing test suite (36 tests) passes
unchanged. The destination/copy list stays self-contained: it is a
real disclosure of `<a>`/`<button>` elements with a roving-focus
pattern, not an ARIA listbox, so headless `Listbox` (which always
renders `role="listbox"` over `role="option"` children) is the wrong
widget for it, not merely an unmigrated one.
