# AGENTS / ssr — ThemeSelect

SSR specifics for `ThemeSelect`. Read
[`../../AGENTS/ssr.md`](../../AGENTS/ssr.md) for the catalog-wide
contract; this file is the per-helper application.

## The directive

`ThemeSelect.tsx` is a **client component**. Every consumer file that
imports it should also carry `"use client"` if it touches React state
or hooks.

If `ThemeSelect.tsx` does not currently start with `"use client"`,
add it at the top:
...
- [ ] `ThemeSelect.tsx` starts with `"use client"`.
