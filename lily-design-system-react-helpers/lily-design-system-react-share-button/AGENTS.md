# AGENTS — ShareButton (React helper)
...
(`../../lily-design-system-svelte-helpers/lily-design-system-svelte-share-button/`),
...
| `ShareButton.tsx` | Implementation. React 19 hooks + TypeScript. |
| `ShareButton.test.tsx` | Vitest spec, mapped to the §7 clauses. |
...
- The list id comes from `React.useId()`, as in the sibling helpers, so
  it survives hydration. `nextShareButtonId()` is exported for parity
...
- Unlike the `*-select` siblings there is no `aria-activedescendant`:
