...
- The list id comes from `React.useId()`, matching the sibling helpers,
  so it survives hydration. `nextShareButtonId()` ships for parity with
...
- Focus into the list moves in a `useEffect` keyed on `open` rather than
  ...
  same role.
- No `aria-activedescendant`, unlike the three `*-select` siblings: the
