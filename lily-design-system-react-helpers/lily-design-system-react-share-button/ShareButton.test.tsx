...
const list = () => document.querySelector(".share-button-list") as HTMLElement;
const statusText = () =>
    (document.querySelector(".share-button-status") as HTMLElement).textContent?.trim();
...
describe("ShareButton — markup contract (§7.1–§7.6)", () => {
    test("§7.1 renders a disclosure button controlling a list", () => {
        render(<ShareButton label="Share" targets={TARGETS} url={URL_UNDER_TEST} />);
        const button = trigger();
        expect(button.tagName).toBe("BUTTON");
        expect(button.getAttribute("type")).toBe("button");
        expect(button.className).toContain("share-button-trigger");
    ...
    test("§7.1 the button renders ↪, hidden from assistive tech", () => {
        render(<ShareButton label="Share" targets={TARGETS} url={URL_UNDER_TEST} />);
        const icon = document.querySelector(".share-button-icon") as HTMLElement;
    ...
    test("§7.1 the root carries the class hook, consumer class, and rest props", () => {
        ...
        expect(root.className).toBe("share-button mine");
    });
    ...
    test("§7.3 destinations are real links, not role=menuitem", async () => {
        ...
        await flush();
        const links =
            document.querySelectorAll<HTMLAnchorElement>(".share-button-target");
    ...
    test("§7.3 newTab: false opts a destination out of target=_blank", async () => {
        ...
        fireEvent.click(trigger());
        await flush();
        const a = document.querySelector(".share-button-target") as HTMLAnchorElement;
    ...
    test("§7.5 the copy item renders only when copyLabel is supplied", async () => {
        const { unmount } = render(
            <ShareButton label="Share" targets={TARGETS} url={URL_UNDER_TEST} />,
        );
        fireEvent.click(trigger());
        await flush();
        expect(document.querySelector(".share-button-copy")).toBeNull();
        unmount();
        ...
        fireEvent.click(trigger());
        await flush();
        const copy = document.querySelector(".share-button-copy") as HTMLElement;
    ...
    test("§7.6 the status region is present, polite, and silent on load", () => {
        render(<ShareButton label="Share" targets={TARGETS} url={URL_UNDER_TEST} />);
        const status = document.querySelector(".share-button-status") as HTMLElement;
...
describe("ShareButton — copy to clipboard (§7.7–§7.10)", () => {
    test("§7.7 copying writes the URL and fires onCopy", async () => {
        ...
        fireEvent.click(trigger());
        await flush();
        fireEvent.click(document.querySelector(".share-button-copy")!);
        await flush();
...
describe("ShareButton — keyboard and dismissal (§7.15–§7.19)", () => {
    ...
    test("§7.15 opening moves focus to the first item", async () => {
        ...
        expect(document.activeElement).toBe(
            list.querySelector(".share-button-target"),
        );
    });
    ...
    test("§7.19 focus leaving the root closes the list", async () => {
        ...
        const { list } = await openList();
        fireEvent.blur(list.querySelector(".share-button-target")!, {
...
describe("ShareButton — url resolution and custom glyph (§7.20–§7.22)", () => {
    test("§7.20 an explicit url prop wins", async () => {
        render(<ShareButton label="Share" targets={TARGETS} url={URL_UNDER_TEST} />);
        fireEvent.click(trigger());
        await flush();
        const href = document
            .querySelector(".share-button-target")!
            .getAttribute("href")!;
    ...
    test("§7.21 with no url prop it falls back to the current page URL", async () => {
        render(<ShareButton label="Share" targets={TARGETS} />);
        fireEvent.click(trigger());
        await flush();
        const href = document
            .querySelector(".share-button-target")!
    ...
    test("§7.22 children replaces the glyph and receives ChildArgs", async () => {
        ...
        expect(document.querySelector(".share-button-icon")).toBeNull();
...
describe("ShareButton — exported helpers (§4.3)", () => {
    test("§4.3 nextShareButtonId mints distinct, prefixed ids", () => {
