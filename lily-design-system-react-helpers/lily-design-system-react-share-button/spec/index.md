...
Single source of truth for the `lily-design-system-react-share-button`
...
[`spec/index.md`](../../../lily-design-system-svelte-helpers/lily-design-system-svelte-share-button/spec/index.md).
...
- `ShareButton.tsx` — the implementation
- `ShareButton.test.tsx` — vitest spec exercising every clause in §7
...
- **A dismissed native sheet is not a failure.** `navigator.share()`
  ...
  the interaction.
- **Real focus, not `aria-activedescendant`.** The sibling `*-select`
...
"title" | "onCopy">`. All three are omitted because the helper redefines
them: `children` is a render prop rather than a node, `title` is share
metadata rather than a tooltip, and `onCopy` reports a completed
clipboard write rather than being the DOM's `ClipboardEvent` handler.

### 4.2 DOM contract

```html
<div class="share-button {className}" ...restProps>
...
`nextShareButtonId()` is exported for parity with the canonical Svelte
...
- Focus moves in a `useEffect` keyed on `open`, after the commit, rather
  than in the handler — the items do not exist in the DOM until the open
  is committed. The Svelte helper's `queueMicrotask` serves the same
  purpose.
- Focus leaving the root is detected with React's `onBlur`, which is the
  ...
  `blur` it bubbles, so the root sees focus leaving any descendant.
- The outside-click listener is attached to `document` only while the
  list is open, and removed on close and on unmount.
- No DOM is read or written during render except `location.href`, which
  is guarded by a `typeof location !== "undefined"` check, so the
...
`ShareButton.test.tsx` asserts every clause below.
...
- Package: lily-design-system-react-share-button
- Version: 0.1.0
- License: MIT
