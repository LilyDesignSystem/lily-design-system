...
| HTML Living Standard — `<select>` | `ThemePicker`, `LocalePicker` default rendering.    |
...
| `<option>` (LocalePicker only) | `lang="{tagFor(locale)}"`  | Component         |
...
`LocalePicker` carries `lang` on each `<option>` so screen
