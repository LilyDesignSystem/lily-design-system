...
describe("ThemeChooser — matchSystemTheme (system-preference detection)", () => {
...
describe("ThemeChooser — detectFromSystem prop", () => {
...
describe("ThemeChooser — markup contract (§4.2, §7.1–§7.5)", () => {
    test("§7.1 renders a button that controls a listbox", () => {
        render(
            <ThemeChooser label="Theme" themesUrl={URL_TRAILING} themes={THEMES} />,
        );
    ...
    test("§7.1 the root is a div carrying the class hook", () => {
        ...
        expect(root.className).toBe("theme-chooser mine");
    });
...
describe("ThemeChooser — spread + custom children (§7.12–§7.13)", () => {
    test("§7.12 extra attributes spread onto the root div", () => {
        ...
        expect(root.tagName).toBe("DIV");
        expect(root.className).toContain("theme-chooser");
    });
