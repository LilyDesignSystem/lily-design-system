# MotionPicker — Specification (React helper)
...
aria-controls>` whose only content is the `aria-hidden` pause-bars
icon (a bundled SVG, replaceable via `children`), and a `<ul class="motion-picker-list"
...
idempotent-apply rule, same bundled-SVG-icon convention (reversed 2026-09-16). It is additive:
