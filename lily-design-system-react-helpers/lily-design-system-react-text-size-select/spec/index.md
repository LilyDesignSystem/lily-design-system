...
and it matches the sibling `theme-select` / `locale-select` specs so all
...
There is deliberately **no detection prop**. Unlike `theme-select`
(`prefers-color-scheme`) and `locale-select` (`navigator.languages`),
...
- **Button + listbox, not a native `<select>`.** The control is an icon
  ...
  from the platform. This is the same shape `theme-select` and
  `locale-select` use, so a row of Lily preference controls in one
...
- **The glyph is `"A"` (U+0041 LATIN CAPITAL LETTER A).** A plain
  ...
  theme-select's ◑, and is the conventional text-size affordance.
...
`<span class="text-size-select-icon" aria-hidden="true">A</span>`. It
...
- **Root element:** `<div className="text-size-select {className}">`.
...
- **Button.** `type="button"`, class hook `text-size-select-button`,
  `aria-label="{label}"`, `aria-haspopup="listbox"`, `aria-expanded`
  tracking open state, `aria-controls` pointing at the listbox id.
- **Glyph.** `<span class="text-size-select-icon" aria-hidden="true">A</span>`
...
- `TextSizeSelect` (the component, both default and named export)
- `sizeName` (the pure helper; the single implementation of the
  title-casing label rule, mirroring `themeName` in theme-select and
...
`TextSizeSelect.test.tsx` must assert every numbered item below. Tests
...
1. Markup shape:
   ...
   2. The button holds `<span class="text-size-select-icon"
      aria-hidden="true">A</span>` (U+0041), equal to the exported
      `LATIN_CAPITAL_LETTER_A`.
   3. The root is a `<div>` whose class is `text-size-select` plus the
      consumer's `className`.
2. `aria-label` is the supplied `label` on **both** the button and the
   listbox.
3. Renders exactly one `.text-size-select-option` per entry in `sizes`,
...
11. *(reserved — no URL construction in this helper; the clause number
    is kept so the numbering aligns with theme-select.)*
...
13. A custom `children` render prop:
    1. Replaces the default glyph inside the button (the
       `.text-size-select-icon` span is absent) and receives `ChildArgs`
       — `value`, `open`, `labelFor`.
...
- Package directory: `lily-design-system-react-helpers/lily-design-system-react-text-size-select/`
- Created: 2026-06-05
