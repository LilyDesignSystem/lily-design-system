...
- **The control is no longer a native `<select>`.** It is now an icon
  ...
  listbox pattern — the shape `theme-select` and `locale-select` already
  ...
  `<div class="text-size-select {className}">` — rest props spread onto
  ...
  - `<button type="button" class="text-size-select-button" aria-label
    aria-haspopup="listbox" aria-expanded aria-controls>` holding
    `<span class="text-size-select-icon" aria-hidden="true">A</span>`;
  - `<ul class="text-size-select-list" role="listbox" aria-label
...
- **The glyph is `"A"` (U+0041 LATIN CAPITAL LETTER A)**, exported from
  `TextSizeSelect.tsx` and the barrel as `LATIN_CAPITAL_LETTER_A`. It is
...
- **`titleCaseSize` is renamed `sizeName`.** Same behaviour
  ...
  theme-select and `localeName` in locale-select so all three catalogs
...
- New class hooks: `text-size-select-button`, `text-size-select-icon`,
  `text-size-select-list`. New attribute hooks for styling:
...
- **No detection prop was added.** Unlike `theme-select`
