...
function getList(): HTMLElement {
    return document.querySelector(".text-size-select-list") as HTMLElement;
}
...
describe("TextSizeSelect — pure helpers", () => {
    ...
    test("labelFor delegates to sizeName, so option text matches it", () => {
        render(<TextSizeSelect label="Text size" sizes={["x-large"]} />);
...
describe("TextSizeSelect — markup contract (§4.2, §7.1–§7.5)", () => {
    ...
    test("§7.1 the root is a div carrying the class hook", () => {
        const { container } = render(
            <TextSizeSelect label="Text size" sizes={SIZES} className="mine" />,
        );
        const root = container.firstElementChild as HTMLElement;
        expect(root.tagName).toBe("DIV");
        expect(root.className).toBe("text-size-select mine");
    });
    ...
    test("§7.5 default labels title-case the slug (no 'default' string)", () => {
        const { container } = render(
            <TextSizeSelect label="Text size" sizes={["small", "x-large"]} />,
        );
...
describe("TextSizeSelect — initial value resolution (§7.6)", () => {
    ...
    test("§7.6 initial value falls back to sizes[0] when 'medium' is absent", async () => {
        render(<TextSizeSelect label="Text size" sizes={["small", "large"]} />);
        await flush();
        expect(document.documentElement.getAttribute("data-text-size")).toBe(
            "small",
        );
...
describe("TextSizeSelect — spread + custom children (§7.12–§7.13)", () => {
    test("§7.12 extra attributes spread onto the root div", () => {
        render(
            <TextSizeSelect label="Text size" sizes={SIZES} data-testid="ts" />,
        );
        const root = screen.getByTestId("ts");
        expect(root.tagName).toBe("DIV");
        expect(root.className).toContain("text-size-select");
    });

    test("§7.13 children replace the button glyph and receive ChildArgs", async () => {
        render(
            <TextSizeSelect label="Text size" sizes={SIZES} value="large">
                {(args: ChildArgs) => (
                    <span
                        data-testid="custom"
