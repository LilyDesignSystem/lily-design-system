...
**Package renamed: `lily-design-system-react-picker-bar` → `@lilydesignsystem/react-picker-bar`.** npm scoped packages
...
relative to `lily-design-system-react-picker-bar`'s last published version (`0.1.0`).
This is this package's first dedicated `CHANGELOG.md`; its prior
history (as `lily-design-system-react-picker-bar`) is recorded in the root
