...
pattern — join the existing free-text search on `/components` in every
example app, all three filters combining as an intersection.

New root canonical data: `components-categories.tsv` (491 rows: slug,
tag, category), generated by the new `bin/generate-component-categories`
from two sources that were already single-source-of-truth — each
component's own "HTML tag" line in `components/{slug}/AGENTS.md`
([AGENTS/headless.md](AGENTS/headless.md)), and
[AGENTS/national-person-identifiers.tsv](AGENTS/national-person-identifiers.tsv)
for the National identifiers bucket. `category` is a best-effort
**structural** grouping (root element + naming-convention suffix
family) — the script's own header says so explicitly, because it
doesn't assert anything about a component's subject matter it can't
actually verify from real metadata. `bin/generate-registries` now
joins that file into every registry it writes — svelte, react, vue,
angular, and html-css-js gained `tag`/`category` fields plus a shared
`CATEGORY_LABEL` map for free; Blazor's `ComponentInfo` record and a
new `ComponentData.CategoryLabel` carry the same; nunjucks-eleventy
isn't part of that generator (its listing is a build-time directory
scan), so its own `src/_data/components.js` reads the root TSV
directly, and a new `src/_data/componentCategories.js` supplies the
label map to templates.

Suffix pattern needs no generated data at all — it's a pure function
of the slug, ported identically (same order, same ids) into every
app's own language. The list mirrors
[AGENTS/components.md](AGENTS/components.md)'s suffix→element table
and compound name-pattern families verbatim; 175 of 491 slugs match
none of them and land in an honest "standalone" bucket rather than a
fabricated family — most of the catalog's leaf components (Alert,
Badge, Hero, …) simply aren't part of a shared naming pattern.

Built canonical-Svelte-first, then ported to React, Vue, Angular,
Blazor, HTML+CSS+JS, and Nunjucks/Eleventy — each port independently
re-verified rather than trusted on its own report: build/typecheck,
the ported suffix-pattern unit tests, and e2e cases per app asserting
against ground truth computed from that app's own data (never
hardcoded counts), run for real against each app's actual built output
in a real browser. Two apps needed more than a straight port:
React's `/components` had no search at all before this (added
together with the two new filters), and Nunjucks/Eleventy's had
neither search nor any client-side JS on the page — its own "pages
work without JS" principle meant building the whole filter panel as a
progressive-enhancement injection (the first module in its existing
`data-module` bootstrapper) rather than server-rendering controls that
would silently do nothing without JavaScript.

Real, unrelated bugs fixed along the way: a stale "407 headless
components" count in Svelte, React, Vue, and Blazor's pages (the
catalog is 491 today) — each now reads its own registry's length
live, so it can't drift again. A concurrent-agent hazard surfaced
during independent verification: the six ports ran in parallel against
the same checkout, and several apps' Playwright configs default to
shared ports (3000, 4173) that a sibling app's own server happened to
be occupying at the same moment, producing a false read the first time
through — resolved per app by re-running against a distinct scratch
port, never by changing any shipped config.

New backlog item logged, not fixed here: P7-T11 — angular-examples has
491 orphaned `src/app/components/*.spec.ts` files copied from
angular-headless but no working vitest/TestBed setup to run them; the
new `vitest.config.ts` this task added is deliberately scoped to the
app's own top-level logic only.
