...
2. **Screen-reader testing.** Automated tooling covers a minority of WCAG success
   ...
   and a [testing matrix](docs/screen-reader-matrix.md) of the twenty components
...
7. **Tell someone.** A link in a design-systems community, a mention in a talk, a
   ...
   [help/outreach/index.md](help/outreach/index.md) if you want to see where the
   gaps are.
...
scratch component — live in [docs/developing.md](docs/developing.md).
...
Then add the row to [components.tsv](components.tsv), implement it in the seven
...
of who runs it: [docs/releasing.md](docs/releasing.md) — version lines, the two
...
1. **An independent accessibility audit.** This is the single highest-value thing
   ...
   split is the [accessibility statement](docs/accessibility-statement.md)). The
   scope an auditor needs is prepared in
   [docs/audit-readiness.md](docs/audit-readiness.md), so funding converts to an
