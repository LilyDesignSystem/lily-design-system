...
The most recent batches (May 2026) added national personal identifier components (now 92 components / 46 identifier types × `-input` + `-view` across 30+ countries, grown from the initial 80/40 shipped with 0.2.0) plus five discourse/medical components (`question`, `answer`, `addressograph-box`, `barcode-image`, `draft`); `qr-code` was renamed `qr-code-image`. See [national-identifiers](../national-identifiers/index.md) for the identifier set.
...
- [ ] `components.tsv` holds exactly 491 rows of `slug\tPascalCase\tdescription`.
...
- [ ] `bin/list-components-as-kebab-case` and `-as-pascal-case` enumerate all 491 entries.
