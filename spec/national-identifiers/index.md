...
- [ ] Every `-input` renders `<input type="text" autocomplete="off">`; every `-view` renders `<span aria-label="…">`. Mostly true but not fully — real, narrow defect found 2026-09-06: in `lily-design-system-html-headless`, 44 of 46 national-identifier `-input` components carry `autocomplete="off"`, but two do not — `united-kingdom-national-health-service-number-input.html` and `united-states-social-security-number-input.html` — even though both components' own `AGENTS.md`/`index.md` docs claim `autocomplete="off"` is present, and the canonical Svelte and Nunjucks ports of the same two components do carry it correctly. The `-view` half checks out (spot-checked `united-states-social-security-number-view.html`: `<span class="..." aria-label="">`).
- [ ] **Every slug and PascalCase name uses the normalized country-exonym + identifier-exonym form, not the code + abbreviation form.** FALSE — real, significant defect found 2026-09-06. At least ~20 of the 46 identifier types use the country **endonym** (native-language name), not the English exonym this criterion and `AGENTS/national-personal-identifier-normalization.md` both specify — e.g. slug `belgique-numero-de-registre-national` (PascalCase `BelgiqueNumeroDeRegistreNationalInput`) uses "Belgique" (French endonym) and "Numéro de Registre National" (French), not the documented "Belgium" + "National Register Number" exonym form; likewise `cesko-rodne-cislo` ("Česko"/"Rodné číslo", not "Czech Republic"/"Birth Number"), plus `cymru-`, `danmark-`, `deutschland-`, `eesti-`, `ellada-`, `espana-`, `hrvatska-`, `island-`, `italia-`, `latvija-`, `lietuva-`, `nederland-`, `norge-`, `polska-`, `slovenija-`, `slovensko-`, `suomi-`, `sverige-`. Confirmed against `AGENTS/national-person-identifiers.tsv`'s own endonym/exonym columns (e.g. row `belgique-numero-de-registre-national`: country endonym "Belgique / België / Belgien" vs. exonym "Belgium"; the slug used the endonym column). The original 2026-05 batch (US, UK, Australia, etc.) does follow the exonym rule; this looks like a later batch of European identifiers that was never reconciled with the documented convention.
- [ ] Each component `index.md` documents country, identifier name, format, validation algorithm, where to find it, and the rendering pattern. Partially true — real gap found 2026-09-06: format and rendering pattern are consistently documented, but a keyword sweep of all 92 component `index.md` files found roughly 80 of 92 with no "where to find it"-style content (the canonical TSV's own "Where to find it" column — e.g. "printed on the NHS medical card, prescriptions...") and roughly 35 of 46 `-input` docs describing only the format shape (e.g. "the last 2 a check digit") without the actual validation algorithm (modulus/checksum method).
