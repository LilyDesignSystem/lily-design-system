...
- [x] `/components` lists all 491 catalog entries and is searchable / filterable. Verified 2026-09-06: `svelte-sveltekit-examples`' `/components` route implements category + suffix-pattern `.filter(...)` logic (plan P6-T5); catalog count of 491 confirmed separately.
- [ ] Each `/components/{slug}` renders a live demo plus canonical metadata. The "canonical metadata" half is well-supported (root `spec/index.md` §11.2: every component has an `html` demo field in every app). The "live demo" half has a confirmed partial defect, however — see the linked finding: `spec/components/index.md`'s acceptance criteria found 46 of 58 `*-view` demo entries in the canonical `component-demos.ts` map render the wrong element shape (`<div>` with no `role="img"` instead of the documented `<span role="img">`), so leaving this open rather than checking it against a demo map with a known-bad shape for a full national-identifier batch.
